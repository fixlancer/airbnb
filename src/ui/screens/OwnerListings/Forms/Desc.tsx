...
const Desc: React.FC<Props> = props => {
    ...
    useEffect(() => {
        ...
        setDesc(formData[0].desc);
        setNetwork(formData[0].wifi[0].network);
        setPass(formData[0].wifi[0].password);

    }, [])

    const changeDesc = (value) => {
        const updatedData = [...formData];
        updatedData[0].desc = value;
    ...
    const saveNetwork = (value) => {
        const updatedData = [...formData];
        updatedData[0].wifi[0].network = value;
        setFormData(updatedData);
    }

    const savePass = (value) => {
        const updatedData = [...formData];
        updatedData[0].wifi[0].password = value;
        setFormData(updatedData);
    }
