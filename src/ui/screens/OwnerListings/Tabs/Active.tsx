...
import { getActiveListing } from '../../../../redux/actions/listingActions';
...
const Active: React.FC<Props> = props => {
  ...
  const { activeListing } = useSelector((state: RootState) => state.listingReducers);
  ...
  useEffect(() => {
    if (!activeListing.listings) {
      setisMiddleLoading(true);
      fetchActive();
    } else {
      setListing(activeListing.listings);
    }
  ...
  const refreshActive = async () => {
    ...
    dispatch(getActiveListing({ page, limit }))
      .then(res => {
