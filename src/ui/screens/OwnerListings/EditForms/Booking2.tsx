...
const Booking2: React.FC<Props> = props => {
    ...
    useEffect(() => {
        setMaxBookingDate(formData.maxBookingDate);
        setMinNights(formData.minNights);
        setMaxNights(formData.maxNights);
        setLongBooking(formData.longBooking);
    }, [])

    const saveMinNights = (value) => {
        const updatedData = {...formData};
            updatedData.minNights = value;
        setFormData(updatedData);
    }

    const saveMaxNights = (value) => {
        const updatedData = {...formData};
            updatedData.maxNights = value;
        setFormData(updatedData);
    }
    ...
    const handleMaxDate = (value) => {
        const updatedData = {...formData};
            updatedData.maxBookingDate = value;
