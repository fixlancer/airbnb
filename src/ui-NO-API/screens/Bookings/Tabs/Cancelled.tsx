import { View, Text, Dimensions, Platform, TouchableOpacity, ScrollView, FlatList, 
RefreshControl, Animated, ActivityIndicator } from 'react-native'
...
import { getCancelledBooking } from '../../../../redux/actions/bookingActions';
...
const Cancelled: React.FC<Props> = props => {

  const { navigation } = props;

  const { cancelledBookings } = useSelector((state: RootState) => state.bookingReducers);
  ...
  useEffect(() => {
    if(!cancelledBookings.bookings){
    setisMiddleLoading(true);
    fetchCancelled();
    } else {
      setBooking(cancelledBookings.bookings);
    }

  }, [])


  const refreshCancelled = async () => {
    ...
    dispatch(getCancelledBooking({page, limit}))
      .then(res => {
  ...
  const renderCancelled = React.useCallback(({ item, index }) => {
